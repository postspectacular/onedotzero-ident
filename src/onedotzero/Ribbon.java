...
public class Ribbon {
	...
	public HashMap<ParticlePole3D, ParticlePole3D> poles = new HashMap<ParticlePole3D, ParticlePole3D>();
	...
	public void applyShake(Vec3D shakeDir, float energy) {
		...
		if (amp > displaceMod.amp) {
			...
			displaceMod.frequency += (energy * MathUtils.PI * 0.03f - displaceMod.frequency) * 0.15f;
		}
	...
	public boolean create(List<ParticlePole3D> poleSet, Vec3D dir,
		...
		List<ParticlePole3D> availablePoles = PoleManager.getFiltered(poleSet,
				PoleManager.MAX_HITCOUNT);
		ParticlePole3D currPole = PoleManager.getOldestPole(availablePoles);
		ParticlePole3D currPoleAlt = PoleManager
				.getLeastUsedPole(availablePoles);
		if (currPole != null || currPoleAlt != null) {
			...
			origPos = startPole;
			List<ParticlePole3D> c1filtered = PoleManager.getFiltered(
					poleManager.c1poles, PoleManager.C1_MAX_HITCOUNT);
			ParticlePole3D extPole = currPole.computeFieldLine(c1filtered, dir,
			...
			if (extPole != null) {
				...
				if (lpg != null) {
					...
					for (int i = 0; !isLoopComplete; i++) {
						...
						ParticlePole3D[] options = lpg
								.getFlowOptionsForPole(currPole);
						float minTheta = MathUtils.TWO_PI;
						Vec2D curr2D = currPole.pos2D;
						for (ParticlePole3D p : options) {
							if (poles.get(p) == null) {
								float theta = MathUtils.abs(curr2D
					...
					dir.normalize().y = -1;
					dir.normalize();
					ParticlePole3D endPole = currPole.computeFieldLine(
							c1filtered, dir, RIBBON_STEP_LENGTH,
							poleManager.bounds, PoleManager.C1_MAX_HITCOUNT);
					if (endPole != null) {
		...
		if (feed.isEnabled) {
			...
			if (u < 1) {
				...
				for (Vec3D v : vertices) {
					boolean isInLetter = (i >= letterStartID && i < letterEndID);
					if (prev != null) {
						u += distances[i]
								* (isInLetter ? SCALED_LETTER_WIDTH
										: SCALED_WIDTH);
					}
					if (u >= 0 && u <= 1.0) {
						float uu = (float) u;
