...
public class Texture {
	...
	protected static final Logger logger = Logger.getLogger(Texture.class
			.getName());
	...
	public Texture(GL gl, GLU glu, PImage img, boolean hasAlpha) {
		...
		maxUV = new Vec2D((float) img.width / width, 1 - (float) img.height
				/ height);
		...
		glu.gluBuild2DMipmaps(GL.GL_TEXTURE_2D, 4, width, height,
				hasAlpha ? GL.GL_RGBA : GL.GL_RGB, GL.GL_UNSIGNED_BYTE, buffer);
	}
	...
	protected ByteBuffer getTextureByteBuffer(PImage img) {
		...
		ByteBuffer rgbaBuffer = BufferUtil.newByteBuffer(width * height
				* bytesPerPixel);
		for (int y = height - 1; y >= 0; y--) {
			for (int x = 0, idx = y * img.width; x < width; x++) {
				int c = (y < img.height && x < img.width) ? img.pixels[idx++]
						: 0;
