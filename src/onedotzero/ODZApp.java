...
public class ODZApp extends PApplet implements InteractionStateListener,
	...
	private static final Logger logger = Logger.getLogger(ODZApp.class
			.getName());
	...
	private void initCamera() {
		...
		camera.zoomSmooth = CONFIG.getFloat("cam.zoom.smooth",
				camera.zoomSmooth);
		camera.rotSmooth = CONFIG.getFloat("cam.rotation.smooth",
				camera.rotSmooth);
	...
	private void initDefaults() {
		BGCOL = TColor.newHex(CONFIG.getProperty("background.col", "1e1e1e"));
		frameRate(CONFIG.getInt("app.framerate", 60));
		numPoles = CONFIG.getInt("defaults.poles.count", numPoles);
		centreExclusion = new Vec3D(0, CONFIG.getFloat(
				"defaults.poles.exclusion.depth", 0.33f), CONFIG.getFloat(
				"defaults.poles.exclusion.height", 0.5f));
		PoleManager.MAX_HITCOUNT = CONFIG.getInt(
				"defaults.poles.letters.maxhitcount", PoleManager.MAX_HITCOUNT);
		PoleManager.C1_MAX_HITCOUNT = CONFIG.getInt(
				"defaults.poles.external.maxhitcount",
				PoleManager.C1_MAX_HITCOUNT);
		maxRibbonCount = CONFIG.getInt("defaults.ribbon.totalmaxcount",
				maxRibbonCount);
		maxRibbonDelay = CONFIG.getInt("defaults.ribbon.spawndelay",
				maxRibbonDelay);
		Ribbon.LOOP_COUNT = CONFIG.getInt("defaults.ribbon.loopcount",
				Ribbon.LOOP_COUNT);
		ribbonWidth = CONFIG.getFloat("defaults.ribbon.width", ribbonWidth);
		letterScale = CONFIG.getFloat("defaults.ribbon.letter.scale",
				letterScale);
		maxScrollSpeed = CONFIG.getFloat("defaults.ribbon.maxscrollspeed",
				0.005f);
		...
		numExportTiles = CONFIG.getInt("defaults.export.numtiles",
				numExportTiles);
	}
	...
	private void initGUI() {
		...
		uiLabelRibbons = ui
				.addTextlabel("uiLabelRibbons", "", UI_X, UI_Y + 200);
		uiLabelRibbons.setVisible(false);

		// ////////////////////////// camera

		Slider s = ui.addSlider("setCamTargetZoom", 0.1f, 6, camera.targetZoom,
				UI_X, UI_Y, 200, 14);
		s.setLabel("Zoom");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setCamZoomSmooth", 0.005f, 0.25f, camera.zoomSmooth,
				UI_X + 400, UI_Y, 200, 14);
		s.setLabel("smooth");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setCamRotSpeedX", -0.05f, 0.05f,
				camera.targetRotSpeed.x, UI_X, UI_Y + 40, 200, 14);
		s.setLabel("X rotation speed");
		s.setTab(UI_CAMERA);
		s = ui.addSlider("setCamRotSpeedY", -0.05f, 0.05f,
				camera.targetRotSpeed.y, UI_X, UI_Y + 80, 200, 14);
		s.setLabel("Y rotation speed");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setCamRotSmooth", 0.005f, 0.25f, camera.rotSmooth,
				UI_X + 400, UI_Y + 40, 200, 14);
		s.setLabel("smooth");
		s.setTab(UI_CAMERA);
		...
		s = ui.addSlider("setTargetPanX", -1000, 1000, camera.pos.x, UI_X,
				UI_Y + 120, 200, 14);
		s.setLabel("cam offset X");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setTargetPanY", -worldBounds.getExtent().y,
				worldBounds.getExtent().y, camera.pos.y, UI_X, UI_Y + 140, 200,
				14);
		s.setLabel("cam offset Y");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setTargetPanZ", -worldBounds.getExtent().z,
				worldBounds.getExtent().z, camera.pos.z, UI_X, UI_Y + 160, 200,
				14);
		s.setLabel("cam offset Z");
		s.setTab(UI_CAMERA);

		s = ui.addSlider("setCamPanSmooth", 0.005f, 0.25f, camera.panSmooth,
				UI_X + 400, UI_Y + 120, 200, 14);
		s.setLabel("smooth");
		s.setTab(UI_CAMERA);
		...
		s = ui.addSlider("setNumExportTiles", 1, 20, numExportTiles,
				UI_X + 100, UI_Y, 100, 14);
		s.setLabel("num tiles");
		s.setTab(UI_EXPORT);

		uiLabelNumTiles = ui.addTextlabel("uiLabelNumTiles", "", UI_X + 100,
				UI_Y + 20);
	...
	private void initMessages() {
		...
		userMessageProvider = new UserMessageProvider(CONFIG.getInt(
				"message.log.count", 10));
		String userCol = CONFIG.getProperty("message.feed.col", "ffff00");
		feedPool.addFeed("user", TColor.newHex(userCol), userMessageProvider);
		messageFormatter = new WordWrapFormatter(CONFIG.getInt(
				"message.wordwrap", 12));
		messageScheduler = new MessageScheduler(CONFIG.getInt(
	...
	private void initOSC() {
		...
		osc = new OSCManager(conf.getProperty("osc.ip", "239.0.0.1"), conf
		...
		if (CONFIG.getBoolean("defaults.poles.custom.enabled", false)) {
			try {
				JAXBContext context = JAXBContext
						.newInstance(CustomPoles.class);
				File file = new File("config/custompoles.xml");
				customPoles = (CustomPoles) context.createUnmarshaller()
						.unmarshal(file);
			} catch (JAXBException e) {
				e.printStackTrace();
			}
	...
	private void initTextures() {
		textureManager = new TextureManager(this, gl, font, CONFIG.getInt(
				"texture.width", 4096));
	}
	...
	public void loadCameraPreset(int id) {
		...
		if (conf.load("config/cam" + id + ".properties")) {
			StringTokenizer st = new StringTokenizer(conf.getProperty(
					"arcball.q_down", "1,0,0,0"), ",");
			...
			st = new StringTokenizer(conf.getProperty("arcball.q_drag",
					"1,0,0,0"), ",");
			arcBall.dragOrientation.set(parseFloat(st.nextToken()),
					parseFloat(st.nextToken()), parseFloat(st.nextToken()),
					parseFloat(st.nextToken()));
			st = new StringTokenizer(conf.getProperty("cam.pan", "0,0,0,0"),
					",");
			...
			st = new StringTokenizer(conf
					.getProperty("cam.rotation", "0,0,0,0"), ",");
			camera.rotation.set(parseFloat(st.nextToken()), parseFloat(st
					.nextToken()), parseFloat(st.nextToken()));
			camera.zoom = camera.targetZoom = conf.getFloat("cam.zoom",
					camera.zoom);
	...
	private void scheduleMessage(String message) {
		...
		while (numLines > 0) {
			ArrayList<MessageLine> msgPart = new ArrayList<MessageLine>(
					maxLines);
	...
	public void setup() {
		...
		initPoles();
		triggerDefaultMessage();
		exporter = new FrameSequenceExporter(sketchPath("export"), APP_NAME,
		...
		initSMS();
		initGUI();
		setAppState(CONFIG.getBoolean("app.mode.identity", false) ? IDENT_STATE
				: IDLE_STATE);
	...
	public void triggerDefaultMessage() {
		...
		String logoText = CONFIG.getProperty("defaults.message.line0",
				"onedotzero");
		float subtitleScale = CONFIG.getFloat(
				"defaults.message.subtitle.scale", 0.5f);
		...
		float totalHeight;
		if (hasSubtitles) {
			totalHeight = subtitleLeading + numSubtitles * subtitleLeading
					* subtitleScale;
		} else {
			totalHeight = alphabet.baseHeight;
		}
		Vec3D offset = new Vec3D(-alphabet.getWidthForString(logoText) / 2, 0,
				-totalHeight / 2);
		...
		if (hasSubtitles) {
			...
			for (int i = 1; i < numSubtitles; i++) {
				String line = CONFIG.getProperty("defaults.message.line" + i,
						"");
	...
	public void updateRibbonTouch() {
		...
		if (isTouching) {
			float radius = CONFIG.getFloat(
					"defaults.ixd.touch.displace.radius", 200);
