...
public class PoleManager {
	...
	public static List<ParticlePole3D> getFiltered(List<ParticlePole3D> poles,
			int maxHitCount) {
		ArrayList<ParticlePole3D> filtered = new ArrayList<ParticlePole3D>(
				poles.size() / 2);
	 ...
	 */
	public final List<ParticlePole3D> c12poles = new ArrayList<ParticlePole3D>();

	/**
	 * Class 1+3: External + inline
	 */
	public final List<ParticlePole3D> c13poles = new ArrayList<ParticlePole3D>();
	...
	protected final HashMap<ParticlePole3D, LetterPoleGroup> messagePoles = new HashMap<ParticlePole3D, LetterPoleGroup>();
	...
	public void init(int numPoles) {
		...
		for (int i = 0; i < numPoles; i++) {
			Vec3D pos = positionStrategy.createPosition(i, numPoles,
					centreExclusion);
			pos.scaleSelf(bounds.getExtent());
			ParticlePole3D p = new ParticlePole3D(pos, -MAX_CHARGE
					* pos.magnitude() / 200);
			c1poles.add(p);
		}
	}
	...
	private void initLetterPoles(Letter letter, Vec3D offset, float scale) {
		...
		for (Vec2D v : letter.outer.points) {
			ParticlePole3D p = new ParticlePole3D(v.to3DXZ().scaleSelf(scale)
					.addSelf(offset), MAX_CHARGE);
			c2poles.add(p);
			lpg.outer.add(p);
